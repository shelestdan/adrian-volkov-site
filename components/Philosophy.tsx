...
const compactFrames = methodFrames.slice(0, 2);
const featuredFrame = methodFrames[2];
const methodNotes = [
      ...
      <div className="shell-inner space-y-[var(--golden-gap)]">
          ...
          <div className="golden-minor min-w-0 space-y-8 lg:story-sticky lg:pr-8">
            ...
            <Reveal delay={0.1} className="max-w-xl">
              <p className="text-[1rem] leading-8 text-[#CCCCCC] sm:text-[1.08rem]">
                The work is simple in principle and demanding in execution:
                clean the movement, sharpen the contact, then make the change
                survive under live point pressure.
              </p>
            </Reveal>
            <Reveal delay={0.14}>
        ...
        <Reveal delay={0.2}>
          <motion.article
            whileHover={reducedMotion ? undefined : { y: -6 }}
            transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}
            className={`story-surface golden-panel ${reducedMotion ? "" : "story-card"} gap-6 p-6 sm:p-8 xl:p-10`}
          >
            <Reveal delay={0.04} className="golden-panel-copy min-w-0 space-y-5 self-start">
              <div className="flex flex-wrap items-center gap-3">
                <span className="media-chip">{featuredFrame.eyebrow}</span>
                <span className="meta-label text-[#CCCCCC]">03 / 03</span>
              </div>
              <p className="meta-label text-[#CCCCCC]">{featuredFrame.kicker}</p>
              <h3 className="max-w-[10ch] font-display text-[clamp(2.3rem,4.1vw,4rem)] leading-[0.94] tracking-[-0.06em] text-white">
                {featuredFrame.title}
              </h3>
              <p className="max-w-[42ch] text-[1rem] leading-8 text-[#CCCCCC] sm:text-[1.06rem]">
                {featuredFrame.copy}
              </p>
              <p className="section-note border-t border-white/8 pt-4 text-[#888888]">
                {methodNotes[2]}
            ...
            <Reveal delay={0.08} className="golden-panel-media min-w-0 self-stretch">
              <MediaFrame
                asset={featuredFrame.media}
                className="aspect-[16/10] w-full lg:min-h-[26rem] xl:h-full xl:min-h-[28rem]"
